...
// ── GET /api/gallery/mine — user's own ───────────────────────────────────────
...
router.post('/', auth, async (req, res) => {
  ...
  // Check user has at least one campaign
  ...
  if (checkError) {
    ...
    // Don't block on check error - let it through
  } else if (!campaignCheck || campaignCheck.length === 0) {
    return res.status(403).json({
      error: 'You need to create a campaign with IVey before submitting to the gallery.'
    });
  }
...
// ── DELETE /api/gallery/:id — admin removes ───────────────────────────────────
