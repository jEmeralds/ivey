...
router.post('/checkout', auth, async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: { metadata: { ivey_user_id: req.userId, plan } },
    });
...
router.put('/bulk', auth, async (req, res) => {
  ...
  const allowed = ['prices', 'kes', 'trial', 'banner', 'limits', 'features'];
  const rows = Object.entries(settings)
    .filter(([key]) => allowed.includes(key))
    .map(([key, value]) => ({ key, value, updated_at: new Date().toISOString() }));

  if (!rows.length) return res.status(400).json({ error: 'No valid settings keys' });
  ...
  try {
    ...
    if (error) throw error;
    console.log(`⚙️  Platform settings updated: ${rows.map(r => r.key).join(', ')}`);
