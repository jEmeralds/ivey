...
const NAV_ITEMS = [
  ...
  { id: 'analytics',  label: 'Social Analysis', icon: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>) },
];
...
const Dashboard = () => {
  ...
  const navigate_section = (id) => {
    // Brands now lives at /brand — navigate there directly
  ...
  const Sidebar = () => (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 transition-transform duration-300 ease-in-out w-60 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:z-auto md:h-auto`}>
            ...
            {NAV_ITEMS.map(item => (
              <button key={item.id} onClick={() => navigate_section(item.id)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  activeSection === item.id && item.id !== 'brands'
                    ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400'
                    : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'
                }`}>
                <span className={`w-5 h-5 flex-shrink-0 ${activeSection === item.id && item.id !== 'brands' ? 'text-emerald-600 dark:text-emerald-400' : ''}`}>
                  {item.icon}
                </span>
                {item.label}
                {item.id === 'campaigns' && campaigns.length > 0 && (
                  <span className="ml-auto text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-2 py-0.5 rounded-full">{campaigns.length}</span>
                )}
                {item.id === 'brands' && (
  ...
  const OverviewSection = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Link to="/new-campaign" className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition-colors shadow-sm text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard value={stats.totalCampaigns}    label="Total Campaigns"    color="bg-emerald-100 dark:bg-emerald-900/30" icon={<svg className="w-6 h-6 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>} />
        <StatCard value={stats.strategiesCreated} label="Strategies Created" color="bg-amber-100 dark:bg-amber-900/30"   icon={<svg className="w-6 h-6 text-amber-600 dark:text-amber-400"   fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>} />
        <StatCard value={stats.contentGenerated}  label="Content Pieces"    color="bg-indigo-100 dark:bg-indigo-900/30"  icon={<svg className="w-6 h-6 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>} />
      </div>
      ...
      {campaigns.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            ...
            <button onClick={() => navigate_section('campaigns')} className="text-emerald-600 dark:text-emerald-400 text-sm font-medium hover:underline">View all →</button>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {campaigns.slice(0, 3).map(c => (
              <div key={c.id} className="px-5 py-4 flex items-center justify-between gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                ...
                <button onClick={() => navigate(`/campaigns/${c.id}`)} className="text-xs text-emerald-600 dark:text-emerald-400 font-medium whitespace-nowrap hover:underline">Open →</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Brand Identity', icon: '🎨', action: () => navigate('/brand'),        color: 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800' },
          ...
          <button key={q.label} onClick={q.action} className={`flex flex-col items-center gap-2 p-4 rounded-xl border text-sm font-medium transition-all hover:scale-105 ${q.color}`}>
        ...
        {campaigns.length === 0 ? (
          <div className="p-16 text-center">
            <div className="w-14 h-14 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="w-7 h-7 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
            </div>
            <h3 className="font-semibold text-gray-900 dark:text-white mb-2">No campaigns yet</h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-5 max-w-xs mx-auto">Create your first campaign and generate AI marketing content in seconds.</p>
            <Link to="/new-campaign" className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition-colors text-sm">Create First Campaign</Link>
          </div>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <button onClick={() => navigate(`/campaigns/${campaign.id}`)} className="font-semibold text-gray-900 dark:text-white hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors text-left">
                        {campaign.name}
                      </button>
                      <StatusBadge campaign={campaign} />
                    </div>
                    <p className="text-gray-500 dark:text-gray-400 text-sm mb-2 line-clamp-1">{campaign.description || 'No description'}</p>
        ...
        <main className="flex-1 p-5 md:p-8 max-w-5xl w-full mx-auto">
          ...
          {activeSection === 'social'    && (
            <div className="space-y-4">
              <div><h1 className="text-xl font-bold text-gray-900 dark:text-white">Social Accounts</h1><p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">Connect your accounts and manage posts</p></div>
              <SocialConnect />
            </div>
          )}
          {activeSection === 'gallery'   && (
            <div className="space-y-4">
              <div><h1 className="text-xl font-bold text-gray-900 dark:text-white">Gallery</h1><p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">Share your best content in the public gallery</p></div>
              <GallerySubmitButton campaigns={campaigns} />
            </div>
          )}
          {activeSection === 'analytics' && (
            <div className="space-y-4">
              <div><h1 className="text-xl font-bold text-gray-900 dark:text-white">Social Analysis</h1><p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">Your social post activity and performance</p></div>
