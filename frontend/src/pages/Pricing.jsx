...
// ─── Checkout modal ───────────────────────────────────────────────────────────
const CheckoutModal = ({ plan, onClose, kesPrice }) => {
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-7 w-full max-w-md shadow-2xl">
        ...
        <div className="mb-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              { id: 'KES', label: '🇰🇪 KES', sub: `KES ${kesPrice.toLocaleString()}/month`, desc: 'Kenya · M-Pesa / Card' },
            ].map(c => (
              <button key={c.id} onClick={() => setCurrency(c.id)}
                className={`p-3 rounded-xl border-2 text-left transition-all ${currency === c.id ? 'border-emerald-500 bg-emerald-500/10' : 'border-gray-700 hover:border-gray-600'}`}>
        ...
        <div className="p-3 bg-gray-800 border border-gray-700 rounded-xl mb-5">
          ...
          <div className="flex flex-wrap gap-2">
            {(currency === 'KES' ? ['M-Pesa', 'Visa / Mastercard', 'Bank Transfer'] : ['Visa / Mastercard', 'Bank Transfer']).map(m => (
        ...
        <ul className="space-y-1.5 mb-5">
          ...
          {plan.features.length > 4 && <li className="text-xs text-gray-600 pl-4">+ {plan.features.length - 4} more features</li>}
        </ul>

        {error && <div className="px-3 py-2 bg-red-900/20 border border-red-800 text-red-400 rounded-xl text-xs mb-4">⚠️ {error}</div>}
        ...
        <button onClick={handlePay} disabled={loading}
          ...
          {loading
            ...
            : <><span>🔒</span><span>Pay {currency === 'KES' ? `KES ${kesPrice.toLocaleString()}` : `$${plan.price}`} / month</span></>
          }
        </button>
...
const Pricing = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('upgrade') === 'success') setTimeout(() => fetchUserPlan(), 2000);
  }, [isAuthenticated]);
  ...
  const { prices, kes, features, banner, trial } = siteSettings;
  ...
  const plans = [
    {
      ...
      annualPrice: 0,
      color:       'gray',
      features: ['Unlimited AI scripts', 'Marketing strategy', 'Gemini AI', '1 brand profile', '3 products', 'Campaign management'],
    },
    {
      id:          'starter',
      name:        'Starter',
    ...
    },
    {
      ...
      price:       prices.creator,
      annualPrice: prices.creator,
      color:       'emerald',
      popular:     true,
    ...
    },
    {
      ...
      price:       prices.studio,
      annualPrice: prices.studio,
      color:       'violet',
      features:    features.studio,
    },
  ];
  ...
  const faqs = [
    { q: 'What payment methods do you accept?',      a: 'We accept M-Pesa, Visa/Mastercard, and bank transfer — covering Kenya and international cards in 150+ countries.' },
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
      ...
      {banner.enabled && banner.message && (
        <div className={`px-4 py-3 text-sm font-medium text-center ${
          banner.type === 'success' ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-b border-emerald-200 dark:border-emerald-800' :
          banner.type === 'warning' ? 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-b border-amber-200 dark:border-amber-800' :
          banner.type === 'error'   ? 'bg-red-500/10 text-red-700 dark:text-red-400 border-b border-red-200 dark:border-red-800' :
          'bg-blue-500/10 text-blue-700 dark:text-blue-400 border-b border-blue-200 dark:border-blue-800'
        }`}>
          {banner.message}
        </div>
      )}
              ...
              <div key={i} className={`relative bg-white dark:bg-gray-800 rounded-2xl border-2 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl flex flex-col ${colors.border}`}
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <button
                    ...
                    {isCurrent ? '✅ Current Plan' : plan.id === 'free' ? (isAuthenticated ? '🚀 Go to Dashboard' : 'Get Started Free') : '🔒 Start Free Trial'}
                  </button>
                </div>
              </div>
            );
      ...
      <section className="border-t border-gray-100 dark:border-gray-800 py-20 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: '⚡', title: 'AI Script Engine',           desc: 'Claude, GPT-4o, Gemini — IVey picks the best model. Scripts scored for viral potential before you see them.',     upto: 'All plans'        },
              { icon: '🎬', title: 'HeyGen Video Production',    desc: 'Pick an avatar, IVey submits your script automatically. Video produced, stored, and ready to distribute.',         upto: 'Creator & Studio' },
              { icon: '🚀', title: 'Multi-Platform Distribution', desc: 'One click posts to TikTok, Instagram, Facebook, LinkedIn. Auto-generates captions for each platform.',            upto: 'Starter and up'   },
            ].map((item, i) => (
      ...
      <section className="border-t border-gray-100 dark:border-gray-800 relative overflow-hidden py-20 px-4">
        ...
        <div className="relative max-w-2xl mx-auto text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-8 leading-relaxed">
            {isAuthenticated ? "You're already in. Go create something viral." : 'Free forever on the Free plan. Upgrade when you need video production and distribution.'}
          </p>
          {isAuthenticated ? (
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link to="/signup" className="inline-flex items-center justify-center gap-2 px-10 py-4 bg-gradient-to-r from-amber-400 to-amber-600 text-white rounded-xl font-black hover:from-amber-500 hover:to-amber-700 transition-all shadow-xl text-sm">
              ...
              <Link to="/login" className="inline-flex items-center justify-center gap-2 px-10 py-4 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition-all text-sm">
                Sign In
              </Link>
            </div>
          )}
      ...
      {activeModal && (
        <CheckoutModal
          ...
          kesPrice={kes[activeModal.id] || activeModal.price * 130}
        />
      )}
    </div>
  );
