...
const VideoScriptCard = ({ campaignId, campaign, showToast, onSave }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setScript(data.script); setExpanded(true);
  ...
  const handleSaveScript = async () => {
    ...
    await onSave({ title: `📝 Script (${fmtDur(duration)}) — ${campaign.name}`, content: script, content_type: 'video_script', format: 'VIDEO_SCRIPT', key: `script_${Date.now()}` });
    setSaved(true);
  };
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500 mt-0.5">{fmtDur(duration)} · ~{wordCount} words · Ready for HeyGen</div>
          </div>
        </div>
        <button onClick={handleGenerate} disabled={generating}
          ...
          {generating ? <><svg className="animate-spin h-3 w-3" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg> Generating...</> : script ? '🔄 Regenerate' : '✨ Generate Script'}
        </button>
      </div>
      {script && (
        <div className="border-b border-gray-700">
          <button onClick={() => setExpanded(!expanded)}
            className="w-full flex items-center justify-between px-5 py-3 hover:bg-gray-700/40 transition-all">
            <span className="text-xs font-semibold text-gray-400">{expanded ? 'Hide script' : 'View script'}</span>
            <span className={`text-gray-500 text-xs transition-transform ${expanded ? 'rotate-180' : ''}`}>▼</span>
          </button>
          {expanded && (
            <div className="px-5 pb-4">
              <div className="bg-gray-700/50 rounded-xl p-4 max-h-96 overflow-y-auto">
          ...
          {videoUrl ? (
            <div className="flex items-center gap-3 p-3 bg-emerald-900/20 border border-emerald-700/40 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <a href={videoUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-emerald-500 hover:text-emerald-400 truncate block mt-0.5">{videoUrl.slice(0, 50)}...</a>
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 py-10 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Back */}
        <button onClick={() => navigate('/dashboard?section=campaigns')} className="text-emerald-500 hover:text-emerald-400 font-medium mb-6 flex items-center gap-2 text-sm">
